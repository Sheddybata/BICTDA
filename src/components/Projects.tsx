        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-8 lg:gap-10">
          {projects.map((project, index) => (
            ...
            >
              ...
              <div className="absolute -top-8 sm:-top-10 md:-top-12 left-1/2 -translate-x-1/2 z-10 w-14 sm:w-16 md:w-24 h-14 sm:h-16 md:h-24 rounded-[10%] bg-gradient-to-br from-bictda-accent/20 to-bictda-light flex items-center justify-center shadow-lg border-4 border-white group-hover:scale-110 transition-transform duration-300">
                <img src={project.image} alt={project.title} className="w-8 h-8 sm:w-10 sm:h-10 md:w-16 md:h-16 object-contain rounded-[10%]" />
              </div>
              <div className="pt-12 sm:pt-16 md:pt-20 pb-4 sm:pb-6 md:pb-8 px-4 sm:px-6 md:px-8 flex flex-col h-full">
                <h3 className="text-lg sm:text-xl md:text-2xl font-extrabold text-bictda-dark mb-2 text-center group-hover:text-bictda-accent transition-colors duration-300">{project.title}</h3>
                <p className="text-bictda-dark/80 mb-4 sm:mb-6 leading-relaxed text-center text-sm sm:text-base md:text-base font-medium flex-1">{project.description}</p>
